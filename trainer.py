...
def get_logits_labels(batch, model, model_obj, device):
    ...
    for i in range(len(queries_data)):  # 遍历每个查询
        ...
        to_add = {gt_tails_texts[i]}
        temp_query = f"{queries_data[i][0]}[SEP]{queries_data[i][1]}"
        next_tail = next_dict[temp_query]
        to_add.update(next_tail)
        known_positives.update(to_add)
        for j in range(M_unique):  # 遍历全局尾实体池
            tail_text = unique_tails_texts[j]
            if ',' in tail_text:
                tail_text_entity = tail_text.split(',')[1]
            else:
                tail_text_entity = tail_text
            if tail_text in known_positives or tail_text_entity in known_positives:
                final_mask[i, j] = False
