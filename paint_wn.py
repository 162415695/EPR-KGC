...
# --- Configuration ---
LOG_DIR = r'D:\PycharmProjects\XPR-KGC\有价值结果\wn_final/'
...
for i, (strategy, rank_list) in enumerate(ranks.items()):
    ...
    for x, y in zip(indices_range, cumulative_data):
        if x % 2 == 1 or x == 10:
             plt.text(x, y, f" {y:.2%}", ha='left', va='center', fontsize=20)
'''
...
plt.xlabel('Entity Rank Position', fontsize=20)
plt.ylabel('Cumulative Proportion (Hits@K)', fontsize=20)
plt.title('Comparison of Link Prediction Strategies (Cumulative Hits@K)', fontsize=20, weight='bold')
plt.legend(title='Strategy', fontsize=20)
plt.grid(True, which='both', linestyle='--', linewidth=2)
plt.tick_params(axis='both', which='major', labelsize=20) # 👈 增大坐标轴数字
