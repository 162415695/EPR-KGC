...
# --- Configuration ---
LOG_DIR = 'log_new/fb_final_2encoder/'
...
print("Loading data...")
try:
    with open(LOG_DIR + 'FB15k237_path_result.json', 'r') as f:
        path_data = json.load(f)
    with open(LOG_DIR + 'FB15k237_hr_result.json', 'r') as f:
        hr_data = json.load(f)
...
for rel in path_data:
    ...
    for idx, item in enumerate(path_data[rel]):
        ...
        path_logits = [val**(1/3)for idx,val in enumerate(item['finish_path_logit'])]
        all_results.append({
